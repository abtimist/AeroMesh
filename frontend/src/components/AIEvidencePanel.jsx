...
const AIEvidencePanel = ({ event, onClose }) => {
  ...
  return (
    <>
      <div 
        className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-[999] transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`} 
        onClick={onClose}
      />
      <div 
      ...
      >
        <div 
          className={`pointer-events-auto w-full max-w-md bg-gray-900/90 backdrop-blur-xl border border-gray-700/50 shadow-2xl overflow-hidden transition-all duration-300 ease-out transform rounded-[16px] ${isVisible ? 'scale-100 opacity-100 translate-y-0' : 'scale-95 opacity-0 translate-y-4'}`}
        >
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-900/50 to-indigo-900/50 px-6 py-5 border-b border-gray-700/50 flex justify-between items-center">
          ...
          <div className="px-6 py-5 border-t border-gray-700/50 bg-gray-900/50">
            <button 
              onClick={onClose}
